...
(defn defbean
  ...
  [id java-class & {:keys [mode s-vals c-args factory init post] :or {mode :prototype}}]
  (try
      (let [setters (valid-bean-setters? java-class s-vals)
...
(defn defabean
  ...
  [java-class & {:keys [s-vals constructor-args factory init post]}]
  (defbean (keyword (gensym "anonymous"))
    java-class :s-vals s-vals :c-args constructor-args :factory factory :init init :post post))
